...
export const quat = {
  ...
  fromAxisAngle(axis: Vec3, rad: number): Quat {
    ...
    const [ax, ay, az] = vec3.normalize(axis);
...
export const vec3 = {
  ...
  length(v: Vec3): number { return Math.hypot(v[0], v[1], v[2]); },
...
export const mat4 = {
  ...
  invert(m: Mat4): Mat4 {
    ...
    const b00=a00*a11-a10*a01, b01=a00*a21-a20*a01, b02=a00*a31-a30*a01,
          b03=a10*a21-a20*a11, b04=a10*a31-a30*a11, b05=a20*a31-a30*a21,
          b06=a02*a13-a12*a03, b07=a02*a23-a22*a03, b08=a02*a33-a32*a03,
          b09=a12*a23-a22*a13, b10=a12*a33-a32*a13, b11=a22*a33-a32*a23;
    let det=b00*b11-b01*b10+b02*b09+b03*b08-b04*b07+b05*b06;
    if (!det) return mat4.identity();
    det = 1/det;
    out[0]=(a11*b11-a21*b10+a31*b09)*det; out[1]=(a21*b08-a01*b11-a31*b07)*det;
    out[2]=(a01*b10-a11*b08+a31*b06)*det; out[3]=(a11*b07-a01*b09-a21*b06)*det;
    out[4]=(a20*b10-a10*b11-a30*b09)*det; out[5]=(a00*b11-a20*b08+a30*b07)*det;
    out[6]=(a10*b08-a00*b10-a30*b06)*det; out[7]=(a00*b09-a10*b07+a20*b06)*det;
    out[8]=(a13*b05-a23*b04+a33*b03)*det; out[9]=(a23*b02-a03*b05-a33*b01)*det;
    out[10]=(a03*b04-a13*b02+a33*b00)*det; out[11]=(a13*b01-a03*b03-a23*b00)*det;
    out[12]=(a22*b04-a12*b05-a32*b03)*det; out[13]=(a02*b05-a22*b02+a32*b01)*det;
    out[14]=(a12*b02-a02*b04-a32*b00)*det; out[15]=(a02*b03-a12*b01+a22*b00)*det;
    return out;
  },

  normalMatrix(model: Mat4): Mat4 { return mat4.transpose(mat4.invert(model)); },

  translation(tx: number, ty: number, tz: number): Mat4 {
    const m = mat4.identity(); m[12]=tx; m[13]=ty; m[14]=tz; return m;
  },

  scaling(sx: number, sy: number, sz: number): Mat4 {
    const m = mat4.identity(); m[0]=sx; m[5]=sy; m[10]=sz; return m;
  },

  rotationX(rad: number): Mat4 {
    const c=Math.cos(rad), s=Math.sin(rad), m=mat4.identity();
